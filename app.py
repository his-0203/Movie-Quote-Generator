...
def home():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if language == 'ko':
                prompt = f"{content_map[selected_genre]['prompt_ko']} {user_input} 스타일: {selected_style}, 연도: {selected_period}"
            else:
                prompt = f"{content_map[selected_genre]['prompt_en']} {user_input} Style: {selected_style}, Decade: {selected_period}"
    ...
    return render_template('index.html', content_map=content_map, selected_genre=selected_genre, selected_style=selected_style, selected_period=selected_period, user_input=user_input, response=response_text, response_image=response_image, show_error_modal=show_error_modal)
